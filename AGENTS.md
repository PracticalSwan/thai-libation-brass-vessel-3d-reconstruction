...
Current phase: **final V7 release / finished-project maintenance.** The V4 image-to-3D pipeline and V139 scan-preserving authoring result are complete: the selected sparse lineage, dense fused cloud, Poisson surface, canonical Blender master, canonical GLB, final report, and professor-facing submission package are the protected finished state.

The current goal is to preserve those verified final deliverables, keep public documentation synchronized with the actual project, and remove only clearly obsolete temporary/intermediate residue. Do not reopen reconstruction, geometry repair, material authoring, or export work unless the user explicitly requests a new technical change.
...
- The selected V7 authoring candidate is `SM_V4_V139_SCAN_PRESERVING_FINAL`. It remains the V94-derived reconstructed mesh: exactly 70 identified synthetic planar V94 cap faces (area `0.3022912687200048 m²`) are removed, one 72-edge physical bottom opening is preserved, the V119 centerline correction remains the orientation reference, the lower band uses only guarded same-object `SM_V4_Scan_CleanHigh` donor evidence, and all later repairs operate on existing reconstructed vertices. No primitive terminal, lathe/profile component, generic side, or rebuilt ornament is used.
- V139 performs the final full-surface cleanup after V136: topology-preserving scan-vertex fairing covers every visible height band, with stronger bounded cleanup in the holder-to-middle transition and tower. Rotational repetition is used only as a robust same-scan per-height profile constraint. Relative to V136, median displacement is about `0.577 mm`, p95 about `4.722 mm`, p99 about `9.055 mm`, with larger movement confined to local scan-defect repair near the terminal. Topology remains 451,312 vertices / 902,838 faces, one connected component, one 72-edge boundary loop, and zero other non-manifold edges.
- The terminal remains reconstructed geometry: folded/cavity-like scan layers were repaired through bounded same-scan profile correction, local fairing, and robust sphere fitting on existing vertices. The final fitted terminal radius is about `40.08 mm`; no sphere primitive or rebuilt finial was added.
...
- The restored bottom opening is intentional open geometry. Its 72 boundary edges are not to be auto-filled merely to make the mesh watertight; its fitted plane remains level (`0.0°`).
...
- The saved V7 authoring master is `reconstruction/v4/blender/best_defensible_v1_trim5_authoring_v7_scan_preserving/Thai_Libation_Vessel_V4_V7_SCAN_PRESERVING_FINAL_V139.blend`, size `604340108` bytes, SHA-256 `951d11585d3b459251bb349acc5515877f4f223fb55189e37d5910f1716ee4f3`.
- The user explicitly authorized final export/promotion on 2026-09-17. V139 is now promoted to `reconstruction/v4/blender/Thai_Libation_Vessel_V4_FINAL.blend` and `reconstruction/v4/blender/Thai_Libation_Vessel_V4_FINAL.glb`. Current canonical SHA-256 values are `5eacf012b9940d1997291c4c1e0720776e2a658bf2c60b3a7e567947695e3436` for the `.blend` and `38a38dc17d23a4a19ecd991b6c4ff8023814b9d17fa6fbb13d0c78fe3e3f1ad6` for the GLB. The canonical `.blend` was subsequently re-saved without changing V139 geometry/material: Blender 5.2 background verification matched the versioned V139 master at 451,312 vertices / 902,838 triangles, `MAT_V4_V115_BrassStatsOnly`, identity transforms, dimensions, and semantic mesh SHA-256 `82348c02e61529d1f19d6e0a11e2c3fcf3228a354d1186c73691d84beb5a393b`. The previous canonical `.blend` byte hash `6663d83303fefac34bbbee85132ad64322e2e9b0bc4db73776f2a882a9d07a0c` remains in Git history. A fresh Blender 5.2 factory-startup import verified the unchanged canonical GLB. Further canonical geometry/export changes require a new explicit scope.
- The professor-facing final handoff is `submission/`. Its user-approved ready state contains the verified V139 GLB, a PLY export of the same geometry, eight 45-degree model views, one turntable contact sheet, `README.txt`, and `SHA256SUMS.txt`. The submission GLB SHA-256 is `38a38dc17d23a4a19ecd991b6c4ff8023814b9d17fa6fbb13d0c78fe3e3f1ad6`; the submission PLY SHA-256 is `42ab10ebec3b69dc8dc7ce28463a9d04fd3f0ecb3b35f888b62bc3c222eb9ff1`. Fresh Blender 5.2 imports of both files must remain 451,312 vertices / 902,838 triangles with matching dimensions; the GLB carries the brass material while the PLY is geometry/normals interchange. The final formatted report is kept as both DOCX and PDF under `docs/`.
- Final cleanup removes superseded V6 authoring/export assets, the standalone V128 checkpoint, pre-V139 diagnostic/repair renders, historical V6 comparison renders/reports, transient `reconstruction/v4/work/` logs, and the duplicate canonical `.blend1` backup from the active working tree. Those historical bytes remain recoverable from Git history where previously committed; do not restore them into the finished tree unless explicitly requested.