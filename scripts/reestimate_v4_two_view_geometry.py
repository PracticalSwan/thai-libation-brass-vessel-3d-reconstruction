...
from v4_repair import copy_frozen_sqlite_snapshot, load_canonical_sqlite_snapshot_manifest
...
def _pose_override_payload(
    ...
    especially for low-parallax pairs.  The disposition matrix is existing
    audit evidence, not a synthesized pose; translation direction comes from
    the current raw-match estimate and E/F are rebuilt consistently with both.
    """
    ...
    translation = np.asarray(estimated.cam2_from_cam1.translation, dtype=np.float64).reshape(3)
    norm = float(np.linalg.norm(translation))
    if not np.isfinite(norm) or norm <= 1e-12:
        raise ValueError("raw-match estimate has no usable translation direction")
    ...
    runtime_angle = float(estimated.cam2_from_cam1.rotation.angle())
    return payload, runtime_angle
...
def _reestimate_pair(
    ...
    if override_matrix is not None:
        payload, runtime_angle = _pose_override_payload(
            estimated,
            payload,
            database.read_camera(int(first.camera_id)),
            database.read_camera(int(second.camera_id)),
    ...
    result.update(
        {
            ...
            "tri_angle_deg": float(estimated.tri_angle),
            "rotation_angle_deg": float(rewritten.cam2_from_cam1.rotation.angle()),
            "mapped_inlier_sha256": hashlib.sha256(np.asarray(mapped_inliers, dtype=np.uint32).tobytes()).hexdigest(),
            "runtime_estimate_rotation_angle_deg": float(runtime_angle),
    ...
    if override_matrix is not None:
        ...
        result["pose_override_source"] = "geo_g10_edge_disposition_v1.independent_audit_record.calibrated_reestimate_rotation_matrix_first_to_second"
    return result
...
def main() -> int:
    ...
    try:
        for first_name, second_name in normalized_pairs:
            records.append(_reestimate_pair(database, first_name, second_name, pycolmap, disposition_records))
    finally:
        database.close()
