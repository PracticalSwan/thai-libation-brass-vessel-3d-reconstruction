...
- Corrected the user-reported fused globe/receiving-bowl relationship using the strong top-oblique source views and the previously disclosed top-only Step-6 mismatch. The bounded correction changes only the upper `bowl_outer` profile above normalized Z 0.385 plus its inferred cavity/rim section; the globe, lower bowl, axis, heights, and component positions remain unchanged. The corrected top bowl/globe radius ratio is **1.14**, annular radial clearance reaches **0.018** at the rim, and rolled-rim clearance is about **0.0194**.
- Final pre-export inspection state now opens in the hero camera with material preview and helper collections hidden; Blender reports no missing external files and `final_validation_report.json` remains accepted with `export_performed=false`.
