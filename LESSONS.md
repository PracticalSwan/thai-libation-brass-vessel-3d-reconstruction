...
- A scan cleanup can become visually wrong even when topology is clean if large vessel regions are replaced with profile/lathe-like geometry. For V7, cleanup must remain visibly traceable to the dense/scan mesh; same-object donor reuse is acceptable only for localized defects.
- Before making a Poisson/scan candidate watertight, inspect source boundary loops. In this project the physical bottom opening remained visible in CleanHigh/raw scan evidence but was flattened by a watertight V94 cap; final scan-preserving repair restores that opening and treats its boundary as intentional.
- Do not rebuild decorative texture from hand-picked reference crops when verified appearance-camera projection is unavailable. Prefer the frozen project-wide appearance statistics and preserve only decorative relief that the scan itself captured.
- Same-object donor transfer still needs geometric guards. Nearest-surface transfer can fold an intentional opening when a boundary vertex matches a nearby interior surface; protect known boundary loops, reject large correspondence jumps, taper marginal displacements, and confirm the result from underside and turntable views before acceptance.
- When a reconstructed high-density scan needs broad visual cleanup, bounded curvature-aware smoothing is safer than profile replacement or remeshing. Lock intentional boundaries, damp structural transition bands, cap displacement against a preserved source candidate, and inspect a full turntable plus close regional views after each pass so cleanup does not drift into generic modeling.
- For rotational scan repair, estimate the object axis from stable global geometry before building any per-height radial constraint. A local center estimated from folded or incomplete sectors can bias the profile and create synthetic rings; use the verified axis, cap corrections, retain rollback meshes, and compare all angular sectors before promotion.
