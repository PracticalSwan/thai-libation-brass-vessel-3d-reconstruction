...
A root-level `submission/` package was created from the verified V139 release. Its user-approved ready state contains a direct copy of the canonical GLB, the Blender 5.2 binary PLY export with triangulated geometry and normals, eight turntable renders, one turntable contact sheet, `README.txt`, and regenerated `SHA256SUMS.txt`. The PLY export is 24,373,990 bytes with SHA-256 `42ab10ebec3b69dc8dc7ce28463a9d04fd3f0ecb3b35f888b62bc3c222eb9ff1`; the GLB remains 27,082,828 bytes with SHA-256 `38a38dc17d23a4a19ecd991b6c4ff8023814b9d17fa6fbb13d0c78fe3e3f1ad6`. Independent factory-startup import checks passed for both: one mesh, 451,312 vertices, 902,838 triangles, matching dimensions and identity transforms; the GLB also carries the expected statistics-only brass material. The final formatted paper is present as both `docs/Thai_Libation_Brass_Vessel_Final_Report.docx` and `docs/Thai_Libation_Brass_Vessel_Final_Report.pdf`.

## Finished-project cleanup (2026-09-20)

A conservative final cleanup removed 624 obsolete/local files totaling about 1.685 GiB while preserving the final deliverables and reproducibility-critical evidence. Removed material includes the superseded V6 authoring/export directory, the standalone V128 checkpoint, pre-V139 V7 diagnostic/repair renders, historical V6 comparison renders and V6-only verification reports, transient `reconstruction/v4/work/` logs, old interim authoring/AO artifacts, and the duplicate canonical `.blend1`. The immutable V4 source and processed sets, selected sparse/dense/Poisson artifacts, current reports, scripts/tests, V139 authoring master/final QA, canonical `.blend`/GLB, setup photos, final DOCX/PDF, and the user-approved submission are retained. Historical committed files remain recoverable from Git history.

The verified cleanup milestone is commit `1d331a72bdc859fd0110899ec1613ab604628cf8` (`chore: finalize cleaned project state`). It was pushed to `origin/main`; a fresh fetch plus `git ls-remote` confirmed the same SHA locally, in `origin/main`, and on GitHub. At that gate the working tree was clean, `git lfs status` had no pending objects, and `git lfs fsck` returned `Git LFS fsck OK`.
