...
User review found the upper receiving-bowl lip too narrow, matching the previously disclosed top-only Step-6 discrepancy. The bounded correction changes only `bowl_outer` above normalized Z **0.385** and the corresponding inferred cavity/rim section; the globe, lower bowl, axis, heights, and component positions are unchanged. The corrected top bowl/globe radius ratio is **1.14**, with annular radial clearance reaching **0.018** at the rim and rolled-rim clearance about **0.0194**. The corrected source-like top-oblique render now shows a continuous annular interior band rather than a fused seam.

`reconstruction/reference_assisted_v2/final/Thai_Libation_Vessel_FINAL.blend` is the current pre-export inspection asset. `final_validation_report.json` is accepted, the file opens in hero-camera material preview with helper collections hidden, and Blender reports no missing external files. The focused Final-V2 Blender/orchestrator suite passes **22/22** after the clearance correction. **Do not export GLB, re-import an export, promote/publish assets, or claim export completion until the user explicitly approves export through `reports/user_export_approval.json`.**
