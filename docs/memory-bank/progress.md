...
The canonical V6 asset remains the verified publication baseline while V7 refinement proceeds separately. The V94 dense diagnostic completed after this documentation pass began. It used a 218-view 1000px GPU-0 PatchMatch workspace with the accepted corrected sparse graph while excluding `geo_g10`, `geo_g11`, and `geo_g12`; PatchMatch completed all 218 photometric and 218 geometric depth/normal maps, mask-aware fusion produced 194,205 points, and a diagnostic Poisson/QA set was generated. V94 is now the selected V7 dense-derived geometry base after V96 completed but measured worse on the 3 cm translated-match ghost metric (`0.17226` versus `0.14061`).
...
The user then requested broad cleanup of all visible surfaces. V128 (`SM_V4_V128_SCAN_SURFACE_POLISHED_FINAL`) applies bounded curvature-aware smoothing/fairing/polish over the reconstructed scan vertices only. Relative to V124, displacement is median ~`0.889 mm`, p90 ~`1.919 mm`, p95 ~`2.261 mm`, p99 ~`2.979 mm`, maximum `9 mm`. Structural height transitions were damped and the 72-edge physical opening was locked. Final topology remains 451,312 vertices / 902,838 faces, one connected component, one intentional boundary loop, zero other non-manifold edges, finite positions, with dimensions about `0.898846 x 0.902123 x 2.102404 m`.

The saved master is `reconstruction/v4/blender/best_defensible_v1_trim5_authoring_v7_scan_preserving/Thai_Libation_Vessel_V4_V7_SCAN_PRESERVING_FINAL_V128.blend`, 554,195,471 bytes, SHA-256 `132d9b0c30849a3af5c346e706a64eefc2f38796f7ff7b7b0649e5224551b57e`. `MAT_V4_V115_BrassStatsOnly` remains the only final material; `photographic_projection_verified=false`. Eight clay turntable views, close base/bowl/globe/neck/terminal/bottom views, and eight brass turntable views were visually reviewed. No generic ornament, side, pedestal, finial, profile/lathe geometry, primitive replacement, GLB export, or canonical promotion was performed.
