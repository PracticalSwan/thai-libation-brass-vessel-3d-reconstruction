...
Final V2 is complete, explicitly user-approved, exported and freshly re-imported. Plan 1 remains frozen at median silhouette IoU **0.901495**, minimum reliable-view IoU **0.856102**, median landmark error **0.011306**, p95 **0.038080**. Plan 2 is accepted after the corrected non-canonical audit (**77 OK / 63 camera failures / 24 mask failures / 0 usable model mismatches**). Plans 3-5 provide 12 source-supported ornament families, practical cleanup, shared UVs, 2048² AO/normal/curvature bakes, polished-brass lookdev and source-camera visual QA. The texture stage remains a truthful component-level photo-informed fallback: **0% direct per-texel projection / 100% inferred fallback fill**.

Two user-review passes corrected the bowl/globe assembly using close oblique photographs while keeping the frozen Plan-1 profile JSON unchanged. The downstream finalization profile uses a compact ellipsoidal globe/shoulder envelope with globe max radius **0.15989**, receiving-bowl/globe max-radius ratio **1.30**, and rolled-rim clearance about **0.0604**, plus a separate narrow lower globe support and two support rings. Accepted Z levels, axis, neck junction, lower bowl profile, pedestal, lid and finial remain preserved.

Canonical assets are `reconstruction/reference_assisted_v2/final/Thai_Libation_Vessel_FINAL.blend` (SHA `70a47d0dd006fdd2a0c65e2d8302ff495f59037c9feb57b118009dd2820d34ed`) and `.glb` (SHA `f0daa50f1198aa9cb79293780611c88af4910a64a13bf79495067d8c724a9f27`). Fresh GLB import preserves **37** meshes, one brass material, three embedded PBR images, exact bounds and silhouette IoU **1.0**; `export_reimport_report.json` and `final_validation_report.json` record `qa_verdict=SHIP` and `export_performed=true`. Final verification passes **369/369** project tests, 297/297 raw integrity and 288/288 selected-input verification. The next phase is coursework demo/report packaging rather than further reconstruction experimentation.
