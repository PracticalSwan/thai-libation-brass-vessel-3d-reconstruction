...
Final V2 has reached the intended **pre-export hard stop**. Plan 1 remains frozen at median silhouette IoU **0.901495**, minimum reliable-view IoU **0.856102**, median landmark error **0.011306**, p95 **0.038080**. Plan 2 is accepted after the corrected non-canonical audit (**77 OK / 63 camera failures / 24 mask failures / 0 usable model mismatches**). Plans 3-5 are implemented with source-supported ornament, practical cleanup, shared UVs, 2048² AO/normal/curvature bakes, photo-informed brass lookdev, and representative source-camera visual QA.

Two user-review passes corrected the bowl/globe assembly using the close oblique photographs while keeping the frozen Plan-1 profile JSON unchanged. The downstream finalization profile now reshapes only the globe/shoulder radial envelope to a compact ellipsoid: globe max radius **0.15989** versus **0.20073** before refinement, receiving-bowl/globe max-radius ratio **1.30**, globe lower radius **0.06715**, and rolled-rim radial clearance about **0.0604**. All accepted Z levels, axis, neck junction, lower bowl profile, pedestal, lid, finial, and component positions are preserved. A separate `SM_Globe_LowerSupport` plus two small support rings now reproduce the narrow lower collar visible inside the bowl in the reference photographs. Zoomed high-oblique, top, globe-close, and quarter renders show the bowl interior as a wide continuous annular band instead of a fused seam.

`reconstruction/reference_assisted_v2/final/Thai_Libation_Vessel_FINAL.blend` is the current pre-export inspection asset and `final_validation_report.json` is accepted with `export_performed=false`. The focused Final-V2 Blender/CV/orchestrator suite passes **92/92** after the second correction. **Do not export GLB, re-import an export, promote/publish assets, or claim export completion until the user explicitly approves export through `reports/user_export_approval.json`.**
