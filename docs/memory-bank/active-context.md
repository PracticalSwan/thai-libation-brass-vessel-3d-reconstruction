...
Final V2 is at the **pre-export user-inspection gate**. Plan 1 remains frozen and accepted at median silhouette IoU **0.901495**, minimum reliable-view IoU **0.856102**, median landmark error **0.011306** object height, and p95 **0.038080**. Corrected Plan 2 re-established report/hash binding and the non-canonical audit records **77 OK**, **63 independently explained camera failures**, **24 mask failures**, and **0 usable model mismatches**, with visibility-derived surface provenance and Step-6 candidate cross-checks retained. Plans 3-5 produced the photographed ornament families, practical cleanup, shared UVs, 2048² AO/tangent-normal/curvature bakes, component-level photo-informed texture fallback, polished-brass material/lookdev, and source-camera QA. Two explicit user-review passes then corrected the bowl/globe assembly while leaving the frozen Plan-1 profile JSON untouched: the downstream finalization profile now uses a compact ellipsoidal globe/shoulder envelope with maximum radius **0.15989** instead of **0.20073**, receiving-bowl/globe max-radius ratio **1.30**, a narrow lower support/collar visible inside the bowl, and about **0.0604** radial clearance at the rolled rim. All component Z levels, axis, neck junction, lower bowl profile, pedestal, lid, finial, and component positions remain unchanged. The focused Final-V2 suite passes **92/92** after the correction. `reconstruction/reference_assisted_v2/final/Thai_Libation_Vessel_FINAL.blend` is the current accepted inspection file and `final_validation_report.json` is accepted with `export_performed=false`. **Do not export/re-import/promote/publish until `reports/user_export_approval.json` explicitly records approval.**
