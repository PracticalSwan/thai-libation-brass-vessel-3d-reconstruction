...
-> source-derived ornament + Blender cleanup/UV/bakes
-> component-level photo-informed material fallback + source-camera visual QA
-> user-approved GLB export + clean re-import verification + final publication
```
...
registered images** and **29,713 points**, a major improvement over the prior
73-image ceiling. The predeclared global gate required at least 274 images, so
...
A later reference-assisted V1 prototype under `reconstruction/reference_assisted/` remains preserved as a visually rejected prototype. Final V2 is now complete and exported after explicit user approval. Plan 1 remains the accepted macro-geometry foundation: median silhouette IoU 0.901495, minimum reliable-view IoU 0.856102, median landmark error 0.011306 object height, p95 0.038080, with bowl/globe/neck/lid/finial visual review passing on the exact candidate. Plan 2's corrected non-canonical audit found 77 usable OK views, 63 independently explained camera failures, 24 mask failures, and zero usable model mismatches. Plans 3-5 produced 12 source-supported ornament families, practical cleanup, shared UVs and 2048-pixel AO/normal/curvature bakes, polished-brass lookdev, and representative source-vs-render QA. The texture stage remains explicitly limited to a component-level photo-informed fallback: exact depth-masked per-texel projection was not run, so direct projection is 0% and inferred/fallback fill is 100%. Two user-review passes corrected the source-visible bowl/globe assembly without altering the frozen Plan-1 profile JSON: the finalization stage uses a compact ellipsoidal globe/shoulder envelope with globe max radius 0.15989, a receiving-bowl/globe max-radius ratio of 1.30, a narrow lower globe support visible inside the bowl, and about 0.0604 radial clearance at the rolled rim. The canonical `.blend` and `.glb` are under `reconstruction/reference_assisted_v2/final/`. Fresh GLB re-import preserved all 37 intended export meshes, one brass material, three embedded PBR images, exact bounds, and silhouette IoU 1.0; `export_reimport_report.json` records `qa_verdict=SHIP`. See [`docs/geometry-ml/final-high-fidelity-presentation-model.md`](docs/geometry-ml/final-high-fidelity-presentation-model.md) for the measured final report and limitations.
...
- no native learned model exists, `learned_recovery_success=false`, and `dense_reconstruction_started=false`.

Measured Step 13 external learned global-recovery result:

- the official CVG LightGlue package was pinned to commit `eb42fee2d71449efb0aa5c10549752b5d75384d8`; the real ALIKED-N16Rot + LightGlue smoke ran on the RTX 5050 Laptop GPU and produced 4,096 features per image plus 2,952 raw matches;
- the exact 2,340-pair learned diagnostic recovered every fixed boundary: 73-74 had 778 qualified candidates, 145-146 had 418, and 203-204 had 745; 8 deterministic bridges were selected per boundary;
...
- pinned the official CVG LightGlue package to commit `eb42fee2d71449efb0aa5c10549752b5d75384d8` and ran ALIKED-N16Rot + LightGlue on CUDA without replacing pyCOLMAP;
- recovered all three exact Step 11 boundaries with 24 selected learned bridges and passed the targeted diagnostic gate;
- ran exactly one 5,574-pair learned mapping attempt; its strongest single model registered 266/288 images with 29,713 points and 1.3748 px mean reprojection error;
- retained the 266-image model as truthful evidence but rejected it under the frozen >=274-image global acceptance gate;
...
## Reproduce Step 13 external learned recovery

Step 13 is intentionally a one-attempt experimental workflow. The measured run
has already been completed, so rerunning `map` would create a new experiment
rather than reproduce the frozen evidence. The staged interface is:

```powershell
python -B run_external_learned_recovery.py --stage capability
python -B run_external_learned_recovery.py --stage diagnose
python -B run_external_learned_recovery.py --stage map
python -B run_external_learned_recovery.py --stage finalize --visual-status failed
```

The published measured outcome is 266/288 registered images, below the fixed
274-image acceptance gate. `step13_summary.json` therefore selects
`reconstruction/sparse/best` as the local fallback and records
